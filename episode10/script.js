...
function updateTimer() {
  ...
  const minutes = Math.floor(elapsedTime / 60).toString().padStart(2, '0'); // Convert elapsed minutes to string (padded with zero if needed)
  const seconds = (elapsedTime % 60).toString().padStart(2, '0'); // Convert elapsed seconds to string (padded with zero if needed)
  timerDisplay.textContent = `${minutes}:${seconds}`; // Update timer display
}
